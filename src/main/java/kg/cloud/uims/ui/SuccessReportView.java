...
public class SuccessReportView extends VerticalLayout implements
	...
	public void buildBody() {
		...
		if (currentUser.hasRole("secretary")) {
			...
			groupSelect.setItemCaptionPropertyId(app
					.getMessage(UimsMessages.GroupName));
			groupSelect.setImmediate(true);
			groupSelect.addListener(this);
			IndexedContainer groupContainer = container.getGroupList_forSelect(app
					.getFacultyId());
		...
		if (currentUser.hasRole("instructor")) {
			...
			groupStudentTable.setContainerDataSource(container
	...
	public void valueChange(ValueChangeEvent event) {
		...
		if (property == groupSelect) {
			...
			if (null == event.getProperty().getValue()) {
				...
				groupStudentTable.setContainerDataSource(container
						.getStudentByGroupContainer(group_id));
