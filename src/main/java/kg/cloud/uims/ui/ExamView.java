...
public class ExamView extends VerticalLayout implements
	...
	public void valueChange(ValueChangeEvent event) {
		...
		if (property == instLessonTable) {
			...
			if (null == event.getProperty().getValue()) {
				...
				String SubjName = instLessonTable
						...
						.getValue().toString();
				getWindow().addWindow(
						new ExamWindow(app, instLessonTable.getValue()
								.toString(), SubjName));
			}
		}
		if (property == deptSelect) {
			...
			if (null == event.getProperty().getValue()) {
				...
				if (year != null) {
					IndexedContainer lessonContainer = container
							.getLessonList_byDept(department_id, year,
									curr_Sem_id);
