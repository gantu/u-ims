			...
			public InputStream getStream() {
				...
				try {
					...
					Paragraph sif = new Paragraph(
							"STUDENT`S SUCCESS CONTROL FORM", big_font);
					...
					Thead.addCell(new Phrase(studDetails.get(0).getGroupName(),
							text_font));
					...
					float[] Tbody_colsWidth = { 0.1f, 1f, 0.3f, 0.4f, 0.3f,
							0.4f, 0.4f, 0.5f };
					...
					if (reportDatasource.size() != 0) {
						for (int i = 0; i < reportDatasource.size(); i++) {
							Item item = reportDatasource
									.getItem(reportDatasource.getIdByIndex(i));

							Tbody.addCell(new Phrase(Integer.toString(i + 1),
									text_font));
							Tbody.getDefaultCell().setHorizontalAlignment(
									Element.ALIGN_LEFT);
							Tbody.addCell(new Phrase(item.getItemProperty(
							...
							Tbody.addCell(new Phrase(
									item.getItemProperty(
											app.getMessage(UimsMessages.StudentMidterm))
											.toString(), text_font));
							Tbody.addCell(new Phrase(item.getItemProperty(
									app.getMessage(UimsMessages.StudentFinal))
									.toString(), text_font));
							Tbody.addCell(new Phrase(item.getItemProperty(
									app.getMessage(UimsMessages.StudentMakeup))
									.toString(), text_font));
							Tbody.addCell(new Phrase(
									item.getItemProperty(
