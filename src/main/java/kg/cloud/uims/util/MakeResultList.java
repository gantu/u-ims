					...
					try {
						...
						if (studLessList.size() != 0) {
							...
							Thead.addCell(new Phrase("Subject: "
									+ studLessList.get(0).getSubjName(),
									in_font));
							...
							for (int i = 0; i < studLessList.size(); i++) {
								String stud_id=Integer.toString(studLessList.get(i).getStudID());
								...
								if ((app.getCurrentExam().getID() == 1)
									...
									if(account.isEmpty() || account.get(0).getFinStatus()!=1){
