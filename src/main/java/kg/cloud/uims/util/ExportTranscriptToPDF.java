...
public class ExportTranscriptToPDF {
	...
	public ExportTranscriptToPDF(final MyVaadinApplication app, String sid,
			...
			IndexedContainer trspDatasource) {
		this.app = app;
		this.studentId = sid;
			...
			public InputStream getStream() {
				...
				try {
					...
					img.setAbsolutePosition(
							document.getPageSize().getWidth() / 5, document
									.getPageSize().getHeight() / 4);
					...
					Paragraph h = new Paragraph(studDetails.get(0)
							.getFaculty_name()
							+ " Faculty - Department of "
							+ studDetails.get(0).getDepartName(), title_font);
					h.setAlignment(Element.ALIGN_CENTER);
					Paragraph st = new Paragraph("Transcript of "
							+ studentFullName + "["
							+ studDetails.get(0).getStRollnum() + "]\t Date:"
							+ df.format(new java.util.Date()), title_font);
					...
					for (int i = 0; i < yearCount.size(); i++) {
						...
						head_table1.addCell(new Phrase("COURSE NAME",
								title_font));
						head_table1.addCell(new Phrase(yearCount.get(i)
								.getYear() + " FALL", title_font));
						head_table1.addCell(new Phrase("C", title_font));
						head_table1.addCell(new Phrase("G", title_font));

						head_table2.addCell(new Phrase("COURSE NAME",
								title_font));
						head_table2.addCell(new Phrase(yearCount.get(i)
								.getYear() + " SPRING", title_font));
						...
						for (int j = 0; j < transcriptDatasource.size(); j++) {
							Item item = transcriptDatasource
									.getItem(transcriptDatasource
											.getIdByIndex(j));
							if (item.getItemProperty(
									...
									&& (item.getItemProperty(
											app.getMessage(UimsMessages.Semester))
											.toString().equals("fall"))) {

								tableIn1.addCell(new Phrase(
										item.getItemProperty(
												app.getMessage(UimsMessages.SubjectCode))
												.toString(), text_font));
								tableIn1.addCell(new Phrase(
										item.getItemProperty(
							...
							if (item.getItemProperty(
									...
									&& (item.getItemProperty(
											app.getMessage(UimsMessages.Semester))
											.toString().equals("spring"))) {

								tableIn2.addCell(new Phrase(
										item.getItemProperty(
												app.getMessage(UimsMessages.SubjectCode))
												.toString(), text_font));
								tableIn2.addCell(new Phrase(
					...
					foot.getDefaultCell().setHorizontalAlignment(
							Element.ALIGN_LEFT);
					foot.addCell(new Phrase("GRADUATE PROJECT TITLE: ",
							text_font));
					foot.addCell(new Phrase("Grade Point Average: "
							+ Math.round((total / creditSum)), text_font));
					document.add(foot);
					...
					footer.addCell(new Phrase("Total Credits Earned: "
							+ Math.round(creditSum), text_font));
					footer.addCell(new Phrase("Head of Student Affairs",
							text_font));
					footer.addCell(new Phrase("", text_font));
					footer.addCell(new Phrase("Date of issue: "
							+ df.format(new java.util.Date()) + "", text_font));
					footer.addCell(new Phrase("", text_font));
					document.add(footer);
