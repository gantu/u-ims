				...
				try {
					...
					try {
						...
						Paragraph sif = new Paragraph(
								"EXAMINATION AVERAGE LIST", big_font);
						...
						if (!studLessList.isEmpty()) {
							...
							float[] Tbody_colsWidth = { 0.1f, 1f, 0.3f, 0.4f,
									0.4f, 0.4f, 0.4f, 0.4f };
								...
								if (examList != null) {
									for (int j = 0; j < examList.size(); j++) {
										if (examList.get(j).getExam().equals("Midterm")) {
											marks[0]=examList.get(j).getPercentage();
										} else if (examList.get(j).getExam()
												.equals("Final")) {
											marks[1]=examList.get(j).getPercentage();
										} else if (examList.get(j).getExam()
												.equals("MakeUp")) {
											marks[2]=examList.get(j).getPercentage();
										}
									}
								}
