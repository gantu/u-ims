...
public class DataContainers {
    ...
    public DataContainers(MyVaadinApplication app) {
        this.app = app;
        studContainer_PROPERTY_NAME = app.getMessage(UimsMessages.RegistrationStudentName);
        studContainer_PROPERTY_SURNAME = app.getMessage(UimsMessages.RegistrationStudentSurname);
        studContainer_PROPERTY_STATUS = app.getMessage(UimsMessages.RegistrationStudentEduStatus);
        ...
        PROPERTY_SUBJECT_REGISTRATION_STATUS = app.getMessage(UimsMessages.SubjectRegistrationStatus);
        ...
        PROPERTY_STUDENT_ATTENDANCE = app.getMessage(UimsMessages.StudentAttandance);
    ...
    public IndexedContainer getStudentByGroupContainer(String groupId) {
        ...
        studentContainer.addContainerProperty(studContainer_PROPERTY_STATUS,
                String.class, null);
        String sem_id = Integer.toString(app.getCurrentSemester().getId());
		String year_id = Integer.toString(app.getCurrentYear().getId());
        try {
            DbStudent dbStudent = new DbStudent();
            dbStudent.connect();
            dbStudent.execSQL_GR_forRegistr(groupId);
            ...
            for (int i = 0; i < studentList.size(); i++) {
                ...
                dbAccounting.execSQL_byID(id, sem_id, year_id);
                ArrayList<StudAccounting> account = dbAccounting.getArray();
                if (account.isEmpty() || account.get(0).getRegStatus()==0) {
    ...
    public IndexedContainer getStudentRegisteredSubjects(String studId,
        ...
        try {
            ...
            for (int i = 0; i < studLessList.size(); i++) {
                ...
                item.getItemProperty(PROPERTY_SUBJECT_REGISTRATION_STATUS).setValue(studLessList.get(i).getStatus());
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public IndexedContainer getStudentSuccessReport(String stID, String semID,
        ...
        container.addContainerProperty(PROPERTY_STUDENT_ATTENDANCE,
                String.class, null);
        String exam_id = Integer.toString(app.getCurrentExam().getID());
        try {
            DbStudLess dbStudLess = new DbStudLess();
            dbStudLess.connect();
            ...
            if (subjectList != null) {
                for (int i = 0; i < subjectList.size(); i++) {
                            ...
                            if (examList.get(j).getExam().equals("Midterm")) {
                                if (midStat == 1) {
                                    item.getItemProperty(
                                            PROPERTY_STUDENT_MIDTERM).setValue(
                                ...
                                if (finStat == 1) {
                                    item.getItemProperty(PROPERTY_STUDENT_FINAL).setValue(
                                            examList.get(j).getPercentage());
                                } else {
                                    item.getItemProperty(PROPERTY_STUDENT_FINAL).setValue("WBI");
                                }
                            } else if (examList.get(j).getExam().equals("MakeUp")) {
                                if (finStat == 1) {
                                    item.getItemProperty(
                                            PROPERTY_STUDENT_MAKE_UP).setValue(
                                            examList.get(j).getPercentage());
                                } else {
                                    item.getItemProperty(
                                            PROPERTY_STUDENT_MAKE_UP).setValue(
                    ...
                    if (item.getItemProperty(PROPERTY_STUDENT_MIDTERM).getValue().toString().equals("WBI")
                            || item.getItemProperty(PROPERTY_STUDENT_FINAL).getValue().toString().equals("WBI")
                            || item.getItemProperty(PROPERTY_STUDENT_MAKE_UP).getValue().toString().equals("WBI")) {
                        item.getItemProperty(PROPERTY_SUBJECT_AVERAGE).setValue("WBI");
                    } else {
                        item.getItemProperty(PROPERTY_SUBJECT_AVERAGE).setValue(Math.round(av));
                    }

                    String att = Integer.toString(dbAttandance.execSQL_All(stID, semID, yearID, subjectList.get(i).getSubjID()));
    ...
    public IndexedContainer getGroupList_forSelect(int f_id) {
        ...
        deptContainer.addContainerProperty(PROPERTY_GROUP_NAME, String.class,
                null);
        ...
        deptContainer.sort(new Object[]{PROPERTY_GROUP_NAME},
                new boolean[]{true});
    ...
    public IndexedContainer getGroupList(int f_id) {
        ...
        deptContainer.addContainerProperty(PROPERTY_DEPARTMENT_CODE,
                String.class, null);
        deptContainer.addContainerProperty(PROPERTY_FACULTY_CODE, String.class,
                null);
    ...
    public IndexedContainer getDeptList(int f_id) {
        ...
        deptContainer.sort(new Object[]{PROPERTY_DEPARTMENT_CODE},
                new boolean[]{true});
        return deptContainer;
        ...
        try {
            ...
            for (int i = 0; i < studLessList.size(); i++) {
                if ((app.getCurrentExam().getID() == 1)
                    ...
                    item.getItemProperty(studContainer_PROPERTY_NAME).setValue(
                            studLessList.get(i).getStudName());
                    item.getItemProperty(studContainer_PROPERTY_SURNAME).setValue(studLessList.get(i).getStudSurname());
                    item.getItemProperty(PROPERTY_GROUP_NAME).setValue(
                            studLessList.get(i).getGrpName());
                    TextFieldValidated attendance = new TextFieldValidated(app);
                    ...
                    if ((dbStudLess.calcAverage_Report(
                        ...
                        String id = Integer.toString(studLessList.get(i).getID());
                        Item item = studListContainer.addItem(id);
                        item.getItemProperty(studContainer_PROPERTY_NAME).setValue(studLessList.get(i).getStudName());
                        item.getItemProperty(studContainer_PROPERTY_SURNAME).setValue(studLessList.get(i).getStudSurname());
                        item.getItemProperty(PROPERTY_GROUP_NAME).setValue(
                                studLessList.get(i).getGrpName());
    ...
    public IndexedContainer getLessonList_byDept(String deptID, String year_id,
            String sem_id) {
