...
public class DbSubjExam extends BaseDb {
	...
	public void execSQL(String subj_id, String yearID, String semID,
			String exam_id) throws SQLException {

		String sql = "select t1.id, t1.name,t1.surname, t1.rollnum, t2.name,t3.exam_name, "
				+ "t4.s_exam_id,t4.stud_less_id,t4.mark,t5.status,t6.name "
				...
				+ "where t2.id=? and t4.exam_id=? and t5.year_id=? and t5.sem_id=? "
		...
		while (result.next()) {
			q.add(new SubjExam(result.getInt("t4.s_exam_id"), result
					.getInt("t4.stud_less_id"), result.getString("t1.name"),
					result.getString("t1.surname"),
					result.getString("t6.name"), result.getString("t2.name"),
					result.getInt("t5.status"), result
							.getString("t3.exam_name"), result
							.getInt("t4.mark"), result.getInt("t1.id"), result
							.getString("t1.rollnum")));
		}
	}

	public void execSQL_sID(String subj_id, String yearID, String semID)
			throws SQLException {
		...
		ResultSet result = stat.executeQuery();
		while (result.next()) {
			q.add(new SubjExam(result.getInt("t4.stud_less_id"), result
					.getString("t1.name"), result.getString("t1.surname"),
					result.getString("t6.name"), result.getString("t2.name"),
					result.getInt("t5.status")));
		}
	}
	...
	public int insertSQL(String StudLessID, String exam_id, String mark)
			throws SQLException {
